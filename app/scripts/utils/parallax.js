...
define(function (require) {
    ...
    var $            = require('jquery'),
        ...
        parallaxPage = function (name, pageHtml) {
            return {
                key:  name,
                page: pageHtml,

                right: function (callback) {
                    var target = parallax.current || this,
                        width = Math.max(window.innerWidth, target.page.outerWidth(true));
                    return this.transition({ left:  width, top: 0 },
                                           { left: -width, top: 0 }, callback);
                },
                left: function (callback) {
                    var width = Math.max(window.innerWidth, this.page.outerWidth(true));
                    return this.transition({ left: -width, top: 0 },
                                           { left:  width, top: 0 }, callback);
                },
                top: function (callback) {
                    var top = Math.max(window.innerHeight, this.page.outerHeight(true));
                    return this.transition({ left: 0, top: -top },
                                           { left: 0, top:  top }, callback);
                },
                bottom: function (callback) {
                    var target = parallax.current || this,
                        top = Math.max(window.innerHeight, target.page.outerHeight(true));
                    return this.transition({ left: 0, top:  top },
                                           { left: 0, top: -top }, callback);
                },
